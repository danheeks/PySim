...
extern dpoint3d ipos, istr, ihei, ifor, ivel, ivelang;
...
void CSolidView::OnPaint()
{
	...
	pixel_data = (unsigned char*)malloc(4 * (m_size.x + 500) * m_size.y);
	voxsetframebuffer((long)pixel_data, 4 * m_size.x, m_size.x, m_size.y);
...
//	glRasterPos3d(-1.0, -1.0, 0);
	...
	drawpoint2d(100, 100, 34535);

	glDrawPixels(m_size.x, m_size.y, GL_RGBA, GL_UNSIGNED_BYTE, pixel_data);
